...
stimuli_params = {
    "pre_stim_resting_sec": 0,#813.666667, # around 12.91 minutes blank screen
    "pre_stim_pause_sec": 0,           # Pause before stimulus
    "post_stim_pause_sec": 0,          # Pause after stimulus
    "inter_block_pause_sec": 1,        # Pause between acquisition blocks
    "n_trials_per_block": 18,           # Trials per block
    "n_rep_stim": 1,                   # Repetitions per stimulus
    "max_n_dots": 5
}
...
# Sort based on the filename
# all_stimuli_files_sorted.sort(key=lambda x: x.name)
all_stimuli_files_sorted.sort(key=lambda x: int(re.search(r'\d+', x.stem).group()) if re.search(r'\d+', x.stem) else float('inf'))
print(all_stimuli_files_sorted)

stimuli = {}
for file_path in all_stimuli_files_sorted:
    df = pd.read_csv(file_path)
    key = file_path #.stem.split("_")[-1]
    stimuli[key] = df

# Assume all CSVs have the same number of frames
conditions = [{"stimulus": key} for key in stimuli.keys()]
trials = data.TrialHandler(nReps=stimuli_params["n_rep_stim"], method="sequential", trialList=conditions, name="trials")
trial_sequence = []
...
try:
    ...
    for idx, trial in enumerate(trials):
        ...
        # ---- Start a new acquisition just before this stimulus ----
        pin_acq.write(1); pin_acq.write(0)
        exp_event_log.append({'event': f'B{block_num}_acq_start_stim{idx}', 'timestamp': exp_clock.getTime()})
        block_event_log.append({'event': f'B{block_num}_acq_start_stim{idx}', 'timestamp': block_clock.getTime()})
        

        # ---- Stimulus presentation (aux line marks stimulus on/off) ----
        ...
        for frame in range(n_frames_trial):
            for dot_idx in range(n_dots):
                ...
                if radius == 0.1:   # time stamp whenever there is a loom
                    pin_aux.write(1)
                    exp_event_log.append({'event': f'B{block_num}_stim{idx}_{stimulus_key}_loom_start', 'timestamp': exp_clock.getTime()})
                    block_event_log.append({'event': f'B{block_num}_stim{idx}_{stimulus_key}_loom_start', 'timestamp': block_clock.getTime()})
                    pin_aux.write(0)
