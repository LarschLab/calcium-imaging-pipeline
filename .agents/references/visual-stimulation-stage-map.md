...
4. PsychoPy and Arduino execution loop
   ...
   - Behavior: experiment runs start acquisition directly after the GUI pre-run checklist; CSV stimuli render as dot fields; MP4 stimuli play via PsychoPy `MovieStim`; block-based runs trigger acquisition for baseline `B0` and each stimulus block, with an inter-block pause after `B0` and between stimulus blocks
