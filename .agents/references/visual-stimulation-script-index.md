...
- `visual_stimulation/dots_runner.py`: shared dots-family owner for PsychoPy execution, Arduino triggers, mock execution, and metadata/log CSV writes (including explicit inter-block pause events in both experiment and block logs for block-based modes).
