...
1. Open the top-level router.
2. Follow its dispatch table.
3. Read the smallest relevant reference doc.
4. Open symbol/stage maps only if needed.
5. Open owning modules before large notebooks/scripts.
6. Open large notebook/script regions only when owner-module context is insufficient.
...
- name the correct workflow-specific handoff log
...
- one index file that routes to workflow-specific logs
- one rolling log per workflow profile

The index file should say which workflow log to open.

Each workflow log should include an append-only template such as:
...
Use these logs only for meaningful handoff value.
...
2. top-level router
3. profile router
4. smallest relevant semantic reference
5. stage map or symbol index only if still needed
6. owning module/package/script
7. large notebook/script region only if owner context is insufficient
8. wrappers only for wrapper behavior
...
- if meaningful work stops with remaining breakage, append the workflow-specific recent-changes log
